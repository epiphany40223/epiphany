...
for f4 in fileList:
    ...
    if (datetime_ts2 < datetime_ts1):
        ...
        print ('f4 less than %s'%(args.restoreTime))
    else:
        print ('f4 greater than  %s'%(args.restoreTime)) 
    ...
    for r1 in reversed(revisionsList):
        log.debug('     revisionList:   %s, %s'%(r1['id'], r1['modifiedTime']))
        
        datetime_ts3 = datetime.strptime(r1['modifiedTime'], '%Y-%m-%dT%H:%M:%S.%fZ')
        
        if (datetime_ts3 < datetime_ts1):
            print ('r1 %s less than %s'%(r1['modifiedTime'],args.restoreTime))
            break;
        else:
            print ('r1 %s greater than  %s'%(r1['modifiedTime'], args.restoreTime)) 
