...
def _normalize_filename(item, src) -> None:
    """
    Represent filenames with a Pathlib object, so that it's equally accessible when running on Windows and Linux.
    """
    if src not in item:
        return
...
# family.  PDS allows there to be multiple HoHs and multiple spouse Members in
...
def _link_families_members(families, members):
    """
    Make a copy because we don't to delete Members from the main/actual list
    """
...
def link_family_or_member_phones(family_or_member, phones, phone_types):
    for p in phones.values():
        fam_or_mem_id = p['Rec']
        if fam_or_mem_id not in family_or_member:
            continue

        f = family_or_member[fam_or_mem_id]
        if 'phones' not in f:
            f['phones'] = list()
...
def _link_member_id(members, member_source_field, member_dest_field, values, value_source_field='Description'):
    for member in members.values():
...
def _link_family_funds(funds, fund_periods, fund_activities, families, all_family_funds, all_family_fund_rates,
...
def load_families_and_members(filename=None, pds=None, active_only=True, parishioners_only=True, log=None):
    """
    Load PDS Families and Members.  Return them as 2 giant hashes, appropriately cross-linked to each other.
    """
    if pds and filename:
    ...
    city_states = PDS.read_table(pds, 'City_DB', 'CityRec', columns=['CityState'], log=log)
    statuses = PDS.read_table(pds, 'StatusType_DB', 'StatusDescRec', columns=['Description', 'Active'], log=log)
    ministries = PDS.read_table(pds, 'MinType_DB', 'MinDescRec', columns=['Description'], log=log)
    talents = PDS.read_table(pds, 'TalType_DB', 'TalDescRec', columns=['Description'], log=log)
    birth_places = PDS.read_table(pds, 'Ask_DB', 'AskRecNum', columns=['AskMemNum', 'BirthPlace'], log=log)
    date_places = PDS.read_table(pds, 'DatePlace_DB', 'DatePlaceRecNum', log=log)
    date_types = PDS.read_table(pds, 'DateType_DB', 'DescRec', columns=['Description'], log=log)
    phone_types = PDS.read_table(pds, 'PhoneTyp_DB', 'PhoneTypeRec', columns=['Description'], log=log)
    req_types = PDS.read_table(pds, 'ReqType_DB', 'ReqDescRec', columns=['Description', 'Expires'], log=log)
    emails = PDS.read_table(
        pds, 'MemEMail_DB', 'EMailRec', columns=['MemRecNum', 'EMailAddress', 'EMailOverMail', 'FamEmail'], log=log
    )
    languages = PDS.read_table(pds, 'LangType_DB', 'LanguageRec', columns=['Description'], log=log)
    mem_phones = PDS.read_table(
        pds, 'MemPhone_DB', 'PhoneRec', columns=['Rec', 'Number', 'PhoneTypeRec', 'Unlisted'], log=log
    )
    mem_keyword_types = PDS.read_table(pds, 'MemKWType_DB', 'DescRec', columns=['Description'], log=log)
    mem_keywords = PDS.read_table(pds, 'MemKW_DB', 'MemKWRecNum', columns=['MemRecNum', 'DescRec'], log=log)
    mem_ministries = PDS.read_table(
        pds, 'MemMin_DB', 'MemKWRecNum', columns=['MinDescRec', 'MemRecNum', 'StatusDescRec'], log=log
    )
    mem_talents = PDS.read_table(
        pds, 'MemTal_DB', 'MemKWRecNum', columns=['TalDescRec', 'MemRecNum', 'StatusDescRec'], log=log
    )
    mem_dates = PDS.read_table(
        pds, 'MemDates_DB', 'MemDateRecNum', columns=['MemRecNum', 'Date', 'DescRec'], log=log
    )
    mem_ethnics = PDS.read_table(pds, 'EthType_DB', 'EthnicDescRec', columns=['Description'], log=log)
    mem_3kw = PDS.read_table(pds, 'User3KW_DB', 'User3DescRec', columns=['Description'], log=log)
    mem_4kw = PDS.read_table(pds, 'User4KW_DB', 'User4DescRec', columns=['Description'], log=log)
    mem_reqs = PDS.read_table(
        pds, 'MemReq_DB', 'MemReqRecNum',
        columns=['MemRecNum', 'ReqDescRec', 'ReqDate', 'ReqResult', 'ReqNote', 'ExpirationDate']
    )

    relationship_types = PDS.read_table(pds, 'RelType_DB', 'RelDescRec', columns=['Description'], log=log)
    marital_statuses = PDS.read_table(pds, 'MemStatType_DB', 'MaritalStatusRec', columns=['Description'], log=log)
    fam_keyword_types = PDS.read_table(pds, 'FamKWType_DB', 'DescRec', columns=['Description'], log=log)
    fam_keywords = PDS.read_table(pds, 'FamKW_DB', 'FamKWRecNum', columns=['FamRecNum', 'DescRec'], log=log)
    fam_status_types = PDS.read_table(pds, 'FamStatType_DB', 'StatDescRec', columns=['Description'], log=log)
    fam_phones = PDS.read_table(
        pds, 'FamPhone_DB', 'PhoneRec', columns=['Rec', 'Number', 'PhoneTypeRec', 'Unlisted'], log=log
    )

    # Descriptions of each fund
    funds = PDS.read_table(
        pds, 'FundSetup_DB', 'SetupRecNum', columns=['FundNumber', 'FundKey', 'FundName'], log=log
    )
    # Each fund also has one or more time periods associated with it
    fund_periods = PDS.read_table(
        pds, 'FundPeriod_DB', 'FundPeriodRecNum',
        columns=['SetupRecNum', 'FundNumber', 'FundYear', 'FundStart', 'FundEnd'], log=log
    )
    # When a Family contributes, each contribution is assocaited with
    # a "funding activity"
    fund_activities = PDS.read_table(
        pds, 'FundAct_DB', 'ActRecNum',
        columns=['FundRecNum', 'GroupName', 'Activity', 'Function', 'GroupOrder', 'pdsorder'], log=log
    )
    ...
    fam_funds = PDS.read_table(
        pds, 'FamFund_DB', 'FDRecNum',
        columns=['FDFamRec', 'FDYear', 'FDFund', 'FDOrder', 'MemRecNum', 'Comment'], log=log
    )
    # Pledging information from the family
    fam_fund_rates = PDS.read_table(
        pds, 'FamFundRate_DB', 'RateRecNum',
        columns=[
            'FundRecNum', 'FDStartDate', 'FDEndDate', 'FDRate', 'FDRateAdj', 'FDNumber', 'FDPeriod', 'FDTotal', 'Batch',
            'BatchDate',
        ]
    )
    # A listing of each individual contribution from each family,
    # cross-referenced to fam_funds.
    fam_fund_history = PDS.read_table(
        pds, 'FamFundHist_DB', 'FERecNum',
        columns=['FEDate', 'ActRecNum', 'FEFundRec', 'FEFamRec', 'FEAmt', 'FEBatch', 'MemRecNum', 'FEChk', 'FEComment'],
        log=log
    )
...
def find_preferred_email(member_or_family):
    """
    If a Member or Family has one or more preferred email addresses, return them (as an array).  If there are no
    preferred email addresses, return None.
    """
...
def find_any_email(member_or_family):
    """
    Return either the Member/Family preferred email addresses, or, if there are no preferred addresses, return the first
    (by sorted order) non-preferred email address (if it exists).  If no email addresses exist, return an empty list.
    """
    mof = member_or_family
