...
# JMS This function is really only relevant to PDS.
# It is not relevant in ParishSoft any more.
...
def calculate_family_values(family, year, log):
    ...
    ps_year = f'{year - 2000:02}'
    fname   = f'{family["lastName"]}, {family["firstName"]}'
    log.debug(f"Calculating family values: {fname}, year: {year} (PS year {ps_year})")
    if 'funds' in family and ps_year in family['funds']:
        year_funds = family['funds'][ps_year]
    else:
        year_funds = dict()
