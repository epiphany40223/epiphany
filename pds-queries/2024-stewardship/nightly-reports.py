...
def upload_csv_to_gsheet(google, google_folder_id, filename, fieldnames, csv_rows, remove_local, log):
    ...
    print(f"JMS Wrote CSV file: {csv_filename}")
    print("JMS Exiting without uploading to Google")
    exit(1)
...
def family_status_csv_report(args, google, pds_families, jotform, log):
    ...
    for row in jotform:
        ...
        csv_data.append({
            'fid' : fid,
            'Stupid Envelope ID' : _dumb(helpers.pkey_url(pds_families[fid]['ParKey'])),
    ...
    print("JMS WE ARE DONE WITH THE REPORT")
    return
