...
def comments(submissions, families, log):
    ...
    with open(filename, 'w') as fp:
        ...
        for fid in sorted(submissions):
            row = submissions[fid]
            comment = row['If you have any other questions, comments, or other information for us, feel free to type them in here.']
            if comment == '':
                continue
...
def member_cell_updates(member_submissions, members, log):
    ...
    for mid, submission in member_submissions.items():
        ...
        jotform_phone = submission['phone']
...
def member_extract_submissions(ws, members, log):
    ...
    def _extract_member_data(ws, mid_fields):
        ...
        name_col = _find_column(ws, 'Household name')
        ...
        for row in range(2, 9999):
            ...
            # See note in read_census_google_spreadsheet() about the
            # oddity in the 2023 Spring census with rows containing
            # blank "active" values.  Bottom line: we treat these rows
            # as errors, and skip them.
            # JMS continue here
            val = ws.cell(row=row, column=active_col).value
            if val is None or len(val) == 0:
                fid = ws.cell(row=row, column=fid_col).value
                name = ws.cell(row=row, column=name_col).value
                log.error(f"Found XLS blank 'active' ({name} / FID {fid}) -- skipping")
                continue
...
def read_census_google_spreadsheet(args, families, log):
    ...
    def _convert(csv_data, log):
        ...
        for row in csv_data:
            ...
            # A weird thing happened in the Spring 2023 census: we got
            # rows in the spreadsheet where the "Is your household
            # still ACTIVE...?" field was blank.  This should not be
            # possible -- this was a mandatory field in Jotform;
            # there's no way it should have been empty.
            #
            # We noticed that this never happened on a first
            # submission for a Family.  I.e., for the N Families that
            # this happened to, there was always a prior, complete
            # submission (i.e., where "active" was not blank).
            #
            # Our best guess is that people left their web browsers on
            # the "you submitted!" jotform page, and if they activated
            # their browser again later, it re-submitted, but somehow
            # missed that "active" field...?  This is weak, but we
            # have no other explanation.
            #
            # Regardless, if the row has a blank "active" field, we're
            # going to treat that row as an error, and skip it.
            if key in row and len(row[key]) == 0:
                log.error(f"Found CSV blank 'active' field row ({row['name']} / FID {row['fid']}) -- skipping")
                continue
