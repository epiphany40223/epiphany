...
def _send_family_emails(message_body, families, submissions,
    ...
    with smtplib.SMTP_SSL(host=smtp_server,
        ...
        for fid in sorted_fids:
            ...
            # JMS DEBUG -- WILL BE FIXED IN SUBSEQUENT PDS DATA
            #
            # Some Families need to be skipped because the only member
            # is unable to handle communication (e.g., those suffering
            # from dementia), and someone (e.g., a son or daughter)
            # has indicated that this person should be removed from
            # parish emails, but not inactivated.
            #
            # It is not yet well-decided how to handle this in PDS
            # data.  So we're just skipping a few families for now.
            if '0206' in family['ParKey'] or '0522' in family['ParKey']:
                log.warning("CODE BLOCKLIST OF FAMILIES TO SKIP! See code for details")
                continue
            ...
            # See if HoH and Spouse are Spanish speakers.  If both
            # have a primary language of Spanish, set the Family
            # census language to Spanish.  Otherwise, set it to
            # English.
            key = 'language'
            spanish = True
            for member in [hoh, spouse]:
                ...
                if key not in member:
                    spanish = False
                    continue
                val = member[key].split('/')[0]
                if val != 'Spanish':
                    spanish = False
            if spanish:
                family['census']['language'] = 'Spanish'

            # Get a list of Member email addresses (from the Family) to whom we
            # should send
            for addr in PDSChurch.family_business_logistics_emails(family):
                to_addresses[addr] = True

            if len(to_addresses.keys()) == 0:
                family['census']['sent_email'] = False
                family['census']['reason not sent'] = 'No Business Logistics or HoH/Spouse emails'
                if log:
                    log.info(f"    *** Have no Business Logistics or HoH/Spouse emails for Family {family['Name']}")
...
def write_results_csv(family_list, filename, extra, log=None):
    ...
    csv_extra_family_fields = {
        ...
        'Language'              : lambda fam: fam['census']['language'],
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        for family in family_list:
            ...
            # Hard code this value (it doesn't flow through the Jotform)
            row['Envelope ID'] = f"' {family['ParKey']}"
...
def main():
    ...
    env_ids = [ family['ParKey'].strip() for family in not_sent ]

    def write_env_ids(name, families, func):
        env_ids = list()
        for family in families:
            if func(family):
                env_ids.append(family['ParKey'].strip())

        pds_str = ', '.join(env_ids)

        filename = f"emails-not-sent-env-ids-{name}-{ts}.txt"
        with open(filename, "w") as fp:
            fp.write(pds_str + "\n")
        print(f"Wrote {filename}")

    write_env_ids('all', not_sent, lambda family: True )
    write_env_ids('english', not_sent, lambda family: family['census']['language'] == 'English' )
    write_env_ids('spanish', not_sent, lambda family: family['census']['language'] == 'Spanish' )
