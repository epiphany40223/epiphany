...
Dim MsgBoxTitle
MsgBoxTitle = "Backup Jordan's Files"
...
Sub printBackupFailed(Str, isWarning)
    ...
    If (isWarning) Then
        LogResults("Backup Warning - " & Str)
    Else
        LogResults("Backup Error - " & Str)
    End If
...
Sub doTheBackup(BackupSrc, BackupDst)
    Dim BackupCmd, q 

    q = Chr(34)
    BackupCmd = "robocopy " & q & BackupSrc & q & " " & q
    BackupCmd = BackupCmd & BackupTarget & "\" & BackupDst & q & " /mir /e /r:0 /w:0"

LogResults(BackupCmd)
...
Sub LogResults(Str)
  ...
  FileName = "c:\Epiphany\BackupLog.txt"

  Set MyFile = oFS.OpenTextFile(FileName, ForAppending, True)

  ' Write to the file.
  MyFile.WriteLine Now() & " - " & Str
  MyFile.Close
End Sub

'BackupTarget = "C:\Epiphany\Backup Files"
BackupTarget = "F:\Backup Files"
Call doTheBackup("D:\Creative Cloud Files", "D_Creative Cloud Files")
Call doTheBackup("C:\Users\jordan\Videos", "C_Videos")
...
LogResults("Backup Completed Successfully !!!")
